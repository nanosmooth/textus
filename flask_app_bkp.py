 ...
 if not session.get('logged_in'):
    ...
    for row in cursor:
        ...
        if row[0] == session.get('username'):
            msg='<div style="float:right;padding:7px;margin:3px 8px 3px 0px;background-color:#BEE1FF;clear:both;max-width:290px;border-radius: 10px;border-bottom: 1px solid #61B6FF;word-wrap: break-word">'+row[1]+'<div style="color: #82C4FD;    font-size: small;    font-family: serif;">'+post_time+'</div></div>'
        else:
            msg='<div style="float:left;padding:7px;margin:3px 0px 3px 8px;background-color:#FEFEFE;clear:both;max-width:290px;border-radius: 10px;border-bottom: 1px solid #BEBEBE;word-wrap: break-word">'+row[1]+'<div style="color: #BBBBBB;    font-size: small;    font-family: serif;">'+post_time+'</div></div>'
        msgarr.append(msg)
...
def send():
    ...
    utc_sec_since_epoch=utc_sec_since_epoch+19800
    post_time=strftime("%I:%M %p %a %d-%b-%Y", gmtime(utc_sec_since_epoch))
    toPut = '<div style="float:right;padding:7px;margin:3px 8px 3px 0px;background-color:#BEE1FF;clear:both;max-width:290px;border-radius: 10px;border-bottom: 1px solid #61B6FF;word-wrap: break-word">'+toPut+'<div style="color: #82C4FD;    font-size: small;    font-family: serif;">'+post_time+'</div></div>'
...
def receive():
   ...
   cursor=conn.execute(sql);
   for row in cursor:
       msg='<div style="float:left;padding:7px;margin:3px 0px 3px 8px;background-color:#FEFEFE;clear:both;max-width:290px;border-radius: 10px;border-bottom: 1px solid #BEBEBE;word-wrap: break-word">'+row[1]+'<div style="color: #BBBBBB;    font-size: small;    font-family: serif;">'+row[3]+'</div></div>'
       msgarr.append(msg)
...
def login():
    ...
    if request.method == 'POST':
        ...
        if request.form['inputUsername'] == 'tejus':
            ...
            app_pass = 'defaultcon'
        elif request.form['inputUsername'] == 'user3':
